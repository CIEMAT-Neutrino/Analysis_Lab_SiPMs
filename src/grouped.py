...
# Prompt the user to choose a SiPM
...
for f,folder in enumerate(chosen_folders):
    ...
    if f == 0: 
        ...
        if hlat_labels != []: hlat = np.concatenate((hlat,h),axis=0)

df_pcbs_ids = df_display(pcbs, labels=pcbs_labels+["IDs","DATE"], name="pcbs", terminal_output=True, save=True,save_path="../data/"+chosen_sipm+"/fit_data/",)
df_sipm_ids = df_display(sipm, labels=sipm_labels+["IDs","DATE"], name="sipm", terminal_output=True, save=True,save_path="../data/"+chosen_sipm+"/fit_data/", 
                        index=["SiPM #1","SiPM #2","SiPM #3","SiPM #4","SiPM #5","SiPM #6"]*len(df_pcbs_ids))
df_pina_ids = df_display(pina, labels=pina_labels+["IDs","DATE"], name="pina", terminal_output=True, save=True,save_path="../data/"+chosen_sipm+"/fit_data/",
                        index=["Pin #1","Pin #2","Pin #3","Pin #4","Pin #5","Pin #6","Pin #7","Pin #8"]*len(df_pcbs_ids))
df_pinr_ids = df_display(pinr, labels=pinr_labels+["IDs","DATE"], name="pinr", terminal_output=True, save=True,save_path="../data/"+chosen_sipm+"/fit_data/", 
                        index=["Pin #1","Pin #2","Pin #3","Pin #4","Pin #5","Pin #6","Pin #7","Pin #8"]*len(df_pcbs_ids))
...
print("PCB")
check_especifications(df_pcbs_ids, df_pcbs_all, pcbs_labels, "../data/"+chosen_sipm+"/fit_data/errors_pcb")

print("SiPMs")
check_especifications(df_sipm_ids, df_sipm_all, sipm_labels, "../data/"+chosen_sipm+"/fit_data/errors_sipm")

if hlat_labels != []:
    print("HLAT")
    check_especifications(df_hlat_ids, df_hlat_all, hlat_labels, "../data/"+chosen_sipm+"/fit_data/errors_hlat")

print("PIN_ANV")
check_especifications(df_pina_ids, df_pina_all, pina_labels, "../data/"+chosen_sipm+"/fit_data/errors_pina")

print("PIN_REV")
check_especifications(df_pinr_ids, df_pinr_all, pinr_labels, "../data/"+chosen_sipm+"/fit_data/errors_pinr")
